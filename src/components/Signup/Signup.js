...
class Signup extends React.Component {
    ...
    componentWillUnmount() {
        document.removeEventListener('keypress', this.handleSignup);
    }
    ...
    handleSignup = (e) => {
        ...
        if(validateData.length !== 0) {
            ...
            let signupUserCheck = this.user_service.addUser(addObj);
            if(signupUserCheck === 0) {
                this.setState({error: true, errorMessage: 'User with this email already exists', loading: false});
            }
            else {
                try {
                    actions.updateUser(addObj);
                ...
                <div className = "row justify-content-center">
                    <div className = "col-md-6 col-sm-6 col-lg-6">
                        <form>
                            ...
                            <div className = "form-group">
                            <input type = "text" name = "name" value = {this.state.name} placeholder = "Full Name" className = "form-control border-0" id = "nameInput" onChange = {this.handleChange}/>
                            </div>
                            ...
                            <button disabled = {this.state.loading} type="submit" className="btn btn-dark" onClick={this.handleSignup}>Sign Up</button>
                        </form>
                    </div>
                </div>
